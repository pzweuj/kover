...
class AppLocalizations {
  ...
  String get downloadAllCoversDescription => _t(
    'If disabled, covers will only be downloaded together with chapters. Covers will still be fetched from the server on demand when not downloaded and a connection is available.',
    '关闭后，封面只会随章节一起下载。未下载封面且网络可用时，仍会按需从服务器获取。',
  );
  String get maxConcurrentDownloads =>
      _t('Max Concurrent Downloads', '最大并发下载数');
  ...
  String get clearDatabaseDescription => _t(
    ...
    '这将清空整个本地数据库，包括所有未同步进度和已下载数据。此操作无法撤销。',
  );
  ...
  String get anonymousDiagnostics =>
      _t('Send anonymous crash reports and diagnostics', '发送匿名崩溃报告和诊断信息');
  String get anonymousDiagnosticsQuestion =>
      _t('Send anonymous crash reports and diagnostics?', '发送匿名崩溃报告和诊断信息？');
  String get anonymousDiagnosticsDescription => _t(
    'Help improve the app by sending anonymous error and performance statistics. The data does not contain any personal information and is uniquely used to improve the app.',
    '发送匿名错误和性能统计信息，帮助改进应用。这些数据不包含任何个人信息，仅用于改进应用。',
  );
  String get diagnosticsCanChange =>
      _t('This can be changed in the settings at any time.', '你可以随时在设置中更改。');
  ...
  String get coverPageDescription => _t(
  ...
  String get highlightResumeParagraph =>
      _t('Highlight Resume Paragraph', '高亮续读段落');
  ...
  String get noActiveSyncOperations =>
      _t('No active sync operations', '没有正在进行的同步操作');
  ...
  String get syncingRecentlyUpdated =>
      _t('Syncing recently updated', '正在同步最近更新');
  ...
  String itemCount(int count) =>
      isZh ? '$count 项' : '$count ${count == 1 ? "item" : "items"}';
  String moreCount(int count) => isZh ? '+$count 更多' : '+$count more';
  String previousChapter(String? title) =>
      isZh ? '上一章：${title ?? ""}' : 'Previous: ${title ?? ""}';
  String nextChapter(String? title) =>
      isZh ? '下一章：${title ?? ""}' : 'Next: ${title ?? ""}';
  String unsupportedFormat(Object format) =>
      _t('Unsupported format: $format', '不支持的格式：$format');
  String refreshingMetadataForSeries(int seriesId) =>
      _t('Refreshing metadata for series $seriesId', '正在刷新系列 $seriesId 的元数据');
  String refreshingCoversForSeries(int seriesId) =>
      _t('Refreshing covers for series $seriesId', '正在刷新系列 $seriesId 的封面');
  String version(String version, String buildNumber) =>
      _t('Version: $version ($buildNumber)', '版本：$version ($buildNumber)');
